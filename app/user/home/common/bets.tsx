...
export default function BetsPage() {
    ...
    const updatePaymentStatus = async (betId: string) => {
        const data = { betId, userId: user?.id };

        try {
            setLoading(true);
            const res = await betApi.updatePyament(data);
            const json = await res.json();

            if (res.status === 1 && json.success) {
            } else {
    ...
    return (
        <>
            ...
            <div className="relative mb-8">
                <Search className="absolute left-5 top-1/2 transform -translate-y-1/2 text-cyan-400 w-5 h-5" />
                <input
                    type="text"
                    placeholder="Search your bets..."
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    className="w-full bg-gradient-to-br from-slate-900/80 to-slate-800/80 border-2 border-cyan-400 rounded-2xl pl-14 pr-4 py-4 text-white placeholder-slate-400 focus:outline-none focus:border-cyan-400 transition-all shadow-sm font-medium"
                />
            </div>

            {/* Tabs */}
            <div className="flex gap-8 mb-8 bg-gradient-to-br from-slate-900/80 to-slate-800/80 p-2 rounded-2xl shadow-2xl w-fit border border-slate-700">
                <button
            ...
            {activeTab === 'active' && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                    {filteredActiveBets.map((bet) => (
                        <div key={bet.id} className="p-6 bg-gradient-to-br from-slate-800/80 to-slate-900/80 rounded-3xl shadow-xl hover:shadow-2xl transition-all transform hover:scale-105 border border-slate-700">
                            <div className="flex items-start justify-between mb-4">
                                <h3 className="text-2xl font-black text-white">{bet.name}</h3>
                                {bet.creator === 'You' && (
                                    <span className="text-xs bg-cyan-100 text-cyan-600 px-3 py-1.5 rounded-xl font-bold">
                                        Creator
                                    </span>
                                )}
                            </div>
                            <div className="space-y-3 mb-5">
                                <div className="flex items-center text-sm text-slate-300 font-semibold">
                            ...
                            <div className="space-y-2">
                                <div className="flex justify-between text-sm font-bold">
                                    ...
                                    <span className="text-cyan-400">{bet.progress||100}%</span>
                                </div>
                            ...
                            <div>
                                <button
                                    onClick={() => router.push(`/user/viewbet/${bet.betId}`)}
                                    className="w-full mt-6 px-4 py-2.5 bg-gradient-to-r from-cyan-500 to-blue-500 text-white rounded-xl font-bold shadow hover:from-cyan-600 hover:to-blue-600 transition-all transform hover:scale-105"
                                >
                                    View Details
                                </button>
                            </div>
            ...
            {activeTab === 'completed' && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                    {filteredCompletedBets.map((bet) => (
                        <div key={bet.id} className="p-6 bg-gradient-to-br from-slate-800/80 to-slate-900/80 rounded-3xl shadow-xl hover:shadow-2xl transition-all border border-slate-700">
                            <h3 className="text-2xl font-black mb-4 text-white">{bet.name}</h3>
                            <div className="space-y-3">
                                ...
                                {bet.winner === 'You' && (
                                    <div className="mt-4 p-5 bg-gradient-to-br from-emerald-400 to-teal-500 rounded-2xl shadow-xl">
                                        <div className="text-3xl font-black text-white flex items-center gap-2">
                                            <Zap className="w-8 h-8" />
                                            +${bet.won}
                                        </div>
                                        <div className="text-sm text-emerald-100 font-bold mt-1">You Won! 🎉</div>
                                    </div>
                                )}
                            </div>
                        </div>
            ...
            {activeTab === 'upcoming' && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                    {upcomingBets.map((bet) => (
                        <div key={bet.betId} className="p-6 bg-gradient-to-br from-slate-800/80 to-slate-900/80 rounded-3xl shadow-xl hover:shadow-2xl transition-all transform hover:scale-105 border border-slate-700">
                            <div className="flex items-start justify-between mb-2">
                                <h3 className="text-2xl font-black text-gray-800">{bet.name}</h3>
                                <span className={`text-xs px-3 py-1.5 rounded-xl font-bold ${bet.participantStatus?.paymentStatus === 'UNPAID'
                            ...
                            <div className="flex gap-2 mt-4">
                                <button
                                    onClick={() => updatePaymentStatus(bet.betId)}
                                    className={`flex-1 px-4 py-2.5 rounded-xl font-bold shadow transition-all ${bet.participantStatus?.paymentStatus === 'UNPAID'
